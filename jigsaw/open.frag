...
// Modify SDF: is_tab=1 adds protrusion, is_tab=0 cuts socket
...
vec4 open_color(vec3 coords_geo, vec3 size_geo) {
    ...
    for (int dy = -1; dy <= 1; dy++) {
        for (int dx = -1; dx <= 1; dx++) {
            ...
            // Scatter direction: outward bias + random
            vec2 cn = (cell + 0.5) / grid;
            vec2 sdir = normalize(cn - 0.5 + (hash2(cell * 7.0 + vec2(13.0, 29.0)) - 0.5) + vec2(0.001));

            // Staggered timing
            float delay = hash(cell * 3.0 + vec2(5.0, 11.0)) * 0.25;
            float pp = clamp((p - delay) / (1.0 - delay), 0.0, 1.0);
            float eased = 1.0 - (1.0 - pp) * (1.0 - pp);

            vec2 offset = sdir * 0.18 * (1.0 - eased);
            float angle = (hash(cell * 11.0 + vec2(7.0, 3.0)) - 0.5) * 0.5 * (1.0 - eased);

            // Transform pixel to piece-local coords (undo offset + rotation)
            ...
            if (sdf < 0.005 && sdf < best) {
                ...
                // Sample texture at piece home position
                ...
                // Anti-aliased edge + outline
                float aa = smoothstep(0.005, -0.005, sdf);
                float outline = smoothstep(0.02, 0.0, abs(sdf)) * 0.4 * (1.0 - eased * 0.9);
                color.rgb *= 1.0 - outline;
                color *= aa * smoothstep(0.0, 0.1, pp);
