// ============================================================
// Dimensional Rift — Open Animation
// p=0: void/rift fully open, p=1: intact window (tear seals)
// ============================================================
...
// Uses inverted progress: tear is fully open at ip=1, sealed at ip=0
float fractureSDF(vec2 uv, float tear_progress) {
    vec2 center = vec2(0.5);
    vec2 d = uv - center;

    // Main fracture spine with domain warping
    ...
    // Tear width shrinks as it seals (tear_progress goes 1 -> 0)
    float tear_width = 0.005 + 0.18 * tear_progress * tear_progress;

    // Extent shrinks as it seals
    float extent = tear_progress * 0.75;
...
float lightning(vec2 uv, float tear_progress, float time_seed) {
    float glow = 0.0;
    vec2 center = vec2(0.5);
    ...
    for (int i = 0; i < 6; i++) {
        ...
        float len = 0.1 + 0.25 * tear_progress;
        ...
        for (int j = 0; j < 4; j++) {
            ...
            glow += exp(-d * d * 8000.0) * flicker * tear_progress;
            prev = next;
        }
    }
    return glow;
...
vec3 raymarchVoid(vec2 uv, float tear_progress) {
    vec3 col = vec3(0.0);
    ...
    // Reverse progress for animation continuity
    float anim_t = (1.0 - tear_progress) * 2.0;

    for (int i = 0; i < 20; i++) {
        vec3 pos = ro + rd * t;

        float density = fbm3(pos * 3.0 + vec3(0.0, 0.0, anim_t));
        ...
        // Particle debris in the void
        float speck = hash3(floor(pos * 12.0 + vec3(anim_t * 1.5)));
        if (speck > 0.97) {
            float speck_bright = (speck - 0.97) * 33.33;
            float speck_flicker = 0.5 + 0.5 * sin(anim_t * 7.5 + speck * 100.0);
...
vec4 open_color(vec3 coords_geo, vec3 size_geo) {
    ...
    // Invert progress: ip=1 means rift fully open, ip=0 means sealed
    float ip = 1.0 - p;

    // Fracture SDF using inverted progress (tear shrinks as p -> 1)
    float sdf = fractureSDF(uv, ip);
    ...
    if (sdf < 0.0) {
        vec3 void_col = raymarchVoid(uv, ip);

        // Lightning along tear edges
        float bolt = lightning(uv, ip, time_seed);
        void_col += vec3(0.5, 0.8, 1.0) * bolt * 0.8;

        // Edge glow inward
        float edge_glow = exp(sdf * 15.0) * ip;
        void_col += vec3(0.3, 0.8, 1.0) * edge_glow * 1.5;

        // Particle debris near edges
        float debris = hash(uv * size_geo.xy * 3.0 + vec2(time_seed * 10.0));
        float debris_mask = smoothstep(-0.15, -0.01, sdf) * ip;
        if (debris > 0.985) {
            float brightness = (debris - 0.985) * 66.67;
            void_col += vec3(0.7, 0.9, 1.0) * brightness * debris_mask;
        }

        // Void fades as the rift seals (p approaches 1)
        float void_alpha = smoothstep(0.0, -0.01, sdf) * smoothstep(1.0, 0.85, p);
    ...
    if (sdf < curl_zone) {
        ...
        curl_t = curl_t * curl_t * ip;

        vec2 grad;
        float eps = 0.005;
        grad.x = fractureSDF(uv + vec2(eps, 0.0), ip) - fractureSDF(uv - vec2(eps, 0.0), ip);
        grad.y = fractureSDF(uv + vec2(0.0, eps), ip) - fractureSDF(uv - vec2(0.0, eps), ip);
    ...
    float chroma_strength = exp(-sdf * 20.0) * ip * 0.012;
